...
from tensorflow.keras.layers import (Conv3D, MaxPooling3D, Flatten, Dense, Dropout, BatchNormalization, Reshape, Layer, Softmax)
...
def create_model_with_attention(input_shape, num_classes):
    ...
    plot_model(
        model,
        to_file='Documentation_Images/AttentionBased3DCNN/AttentionBased3DCNN_Model.png',
        show_shapes=True,
        show_layer_names=True
    )

    visualkeras.layered_view(model, to_file='Documentation_Images/AttentionBased3DCNN/AttentionBased3DCNN_3DModel.png',legend=True)
    return model
...
def plot_training_history(history):
    ...
    plt.savefig('Documentation_Images/AttentionBased3DCNN/AttentionBased3DCNN_TrainingHistory.png')
    plt.close()
...
def plot_loss_history(history):
    ...
    plt.savefig('Documentation_Images/AttentionBased3DCNN/AttentionBased3DCNN_LossHistory.png')
    plt.close()
...
def train_model(preprocessed_folder, test_folder, test_size=0.2, batch_size=4, epochs=100, test_eval_interval=2):
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath='models/AttentionBased3DCNN_BestModel.h5',  # File to save the best model
    ...
    history = model.fit(
        ...
        # callbacks=[test_accuracy_callback]  # Add the custom callback here
    )
...
model, history = train_model(preprocessed_folder, test_folder, epochs=50, test_eval_interval=1)  # Specify epochs here
